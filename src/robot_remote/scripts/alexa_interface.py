...
from robot_remote.msg import RobotTaskAction, RobotTaskGoal
import rospy
import threading
import actionlib

threading.Thread(target=lambda: rospy.init_node('alexa_interface', disable_signals=True)).start()
client = actionlib.SimpleActionClient('task_server', RobotTaskAction)
...
class PickIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        speech_text = "Picking the object"

        handler_input.response_builder.speak(speech_text).set_card(
            SimpleCard("Pick", speech_text)).set_should_end_session(
            True)

        goal = RobotTaskGoal(task_number=1)
        client.send_goal(goal)
...
class SleepIntentHandler(AbstractRequestHandler):
    ...
    def handle(self, handler_input):
        # type: (HandlerInput) -> Response
        speech_text = "Entering sleep mode"

        handler_input.response_builder.speak(speech_text).set_card(
            SimpleCard("Sleep", speech_text)).set_should_end_session(
            True)

        goal = RobotTaskGoal(task_number=2)
        client.send_goal(goal)
...
skill_adapter = SkillAdapter(
    skill=skill_builder.create(), skill_id="amzn1.ask.skill.d76c895d-2909-4ce6-be4f-5138910eaa3d",
    app=app)
